...
problem = de.IVP(domain, variables=['n', 'psi', 'vx', 'vy'])
...
#problem.add_equation("dt(Lap(psi)) + Ad*(psi - n) - Mu*Lap(Lap(psi)) = dy(psi)*dx(Lap(psi)) - dx(psi)*dy(Lap(psi))")
#problem.add_equation("dt(n) + Ad*(psi - n) - D*Lap(n) + Ka*dy(psi)= dy(psi)*dx(n) - dx(psi)*dy(n)")
problem.add_equation("dt(Lap(psi)) + Ad*(psi - n) + Mu*Lap(Lap(Lap(psi))) = -(vx*dx(Lap(psi)) + vy*dy(Lap(psi)))")
problem.add_equation("dt(n) + Ad*(psi - n) + D*Lap(Lap(n)) + Ka*dy(psi)= -(vx*dx(n) + vy*dy(n))")
...
if not pathlib.Path('restart.h5').exists():
    ...
    for wave in range(20):
        ...
        amplitude = np.sqrt(np.random.chisquare(2))*1e-1*5
    ...
    for wave in range(5):
        ...
        phase = np.random.rand()*2*np.pi
        amplitude = np.sqrt(np.random.chisquare(2))*4e-1*5

        if k == 0:
            continue

        base = base + amplitude*np.cos(k*x + phase)
...
try:
    ...
    while solver.proceed:
        ...
        if (solver.iteration-1) % 10 == 0:
            ...
            logger.info('Average timestep (ms): %f' % ((next_time-curr_time)/10.0 * 1000.0))
